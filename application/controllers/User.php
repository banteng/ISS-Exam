...
class User extends CI_Controller {
	...
	private function header($data){
		$this->load->view('_template/header_new', $data);
	}
	...
	function notfound(){
     ...
     public function ujian($id_ujian){
          ...
          if ($cek_sdh_selesai < 1) {
               ...
               if ($cek_sdh_ujian->num_rows() < 1) {
                    ...
                    $q_ambil_soal = $this->db->query('SELECT * FROM ikut_ujian WHERE id_ujian = '.$id_ujian.' AND id_siswa = '.$data['sess_id'].' ORDER BY RAND() ')->row();
     ...
     public function setting(){
     	$id = ['id_siswa' => $this->session->id];
     	$data['ar'] = $this->m_user->cekdefpass($id)->row_array();
          $data['log'] = $this->m_user->log_auth($this->session->userdata('nis'));
          //echo $this->db->last_query();die;
     	$data['title'] = 'Setting | International Student Service';
          $data['judul'] = 'Setting';

     	$this->header($data);
     	$this->load->view('setting_new');
     	$this->load->view('_template/footer_new');
     }
     function gantipass(){
     	...
     	if ($password != $cek['password']) {
     		# code...
     		$this->session->set_flashdata('repass', 'Old passwords do not match !');
     		redirect('setting');
     	}
     	else{
     		$data = ['password' => $passwordBaru];
     		$this->m_user->gantipass($data, $whr);
               $this->session->set_flashdata('repass', 'Password successfully change');
               redirect('setting');
     	}
     }
